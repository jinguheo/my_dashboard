    ...
    <div className="flex-1 overflow-hidden flex">
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-lg mx-auto space-y-4">
          <div className="bg-surface rounded-xl p-3 space-y-3">
            <div className="flex items-center gap-2">
              <select
                ...
                className="flex-1 bg-surface-card text-gray-200 text-xs rounded-lg px-2.5 py-2 outline-none border border-surface-border"
              >
              ...
              <button onClick={loadAllGcal} disabled={gcalLoading} className="text-xs px-2.5 py-2 bg-surface-card hover:bg-surface-hover text-gray-300 rounded-lg disabled:opacity-50">
                {gcalLoading ? '로딩...' : '새로고침'}
              </button>
            </div>
            {selectedAccount && (
              <div className="flex items-center justify-between">
                <span className={`text-xs ${tokens[selectedAccount.id] ? 'text-green-400' : 'text-gray-500'}`}>
                  {tokens[selectedAccount.id] ? '연결됨' : '연결 안 됨'}
                </span>
                {tokens[selectedAccount.id] && selectedAccount.provider !== 'mcp' ? (
                  <button onClick={() => handleGcalDisconnect(selectedAccount)} className="text-xs text-red-400 hover:text-red-300">연결 해제</button>
                ) : (
          ...
          <div className="flex items-center justify-between">
            <button onClick={prevMonth} className="w-8 h-8 rounded-lg bg-surface hover:bg-surface-hover text-gray-400 hover:text-white">‹</button>
            <h2 className="text-base font-bold text-white">{year}년 {month + 1}월</h2>
            <button onClick={nextMonth} className="w-8 h-8 rounded-lg bg-surface hover:bg-surface-hover text-gray-400 hover:text-white">›</button>
          </div>

          <div className="grid grid-cols-7">
            {DAYS.map((d, i) => <div key={d} className={`text-center text-xs font-medium py-1 ${i === 0 ? 'text-red-400' : i === 6 ? 'text-blue-400' : 'text-gray-500'}`}>{d}</div>)}
          </div>
          ...
          <div className="grid grid-cols-7 gap-0.5">
            {days.map((day, i) => {
              ...
              return (
                ...
                >
                  {day}
                  {hasEvent(dateStr) && <span className={`absolute bottom-1 w-1 h-1 rounded-full ${isSelected ? 'bg-white' : 'bg-accent-light'}`} />}
                </button>
              )
            })}
          </div>
      ...
      <div className="w-72 shrink-0 bg-[#0d0d1a] border-l border-surface-border flex flex-col">
        ...
        <div className="flex-1 overflow-auto p-3 space-y-2">
          {gcalEvtsForDay.map(e => (
            <a key={`${e.accountId}-${e.id}`} href={e.htmlLink} target="_blank" rel="noopener noreferrer" className="flex items-start gap-2 bg-surface rounded-lg p-2.5 hover:bg-surface-hover group">
              <span className="w-2 h-2 rounded-full shrink-0 mt-1" style={{ background: gcalColor(e.colorId) }} />
              <div className="flex-1 min-w-0">
                <p className="text-[10px] text-gray-500">{e.accountName}</p>
          ...
          {localEvts.map(e => (
            <div key={e.id} className="flex items-start gap-2 bg-surface rounded-lg p-2.5 group">
              ...
              <button onClick={() => calendar.remove(e.id)} className="opacity-0 group-hover:opacity-100 text-gray-600 hover:text-red-400 text-sm">x</button>
            </div>
          ))}

          {localEvts.length === 0 && gcalEvtsForDay.length === 0 && <p className="text-xs text-gray-600 text-center mt-4">일정이 없습니다.</p>}
        </div>

        <form onSubmit={handleAdd} className="p-3 border-t border-surface-border space-y-2">
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="일정 추가..." className="w-full bg-surface rounded-lg px-3 py-2 text-xs text-gray-200 placeholder-gray-600 outline-none focus:ring-1 focus:ring-accent/50" />
          <div className="flex gap-2">
            <input type="time" value={time} onChange={e => setTime(e.target.value)} className="flex-1 bg-surface rounded-lg px-2 py-1.5 text-xs text-gray-300 outline-none" />
            <div className="flex gap-1">
              {COLORS.map(c => <button key={c} type="button" onClick={() => setColor(c)} className={`w-4 h-4 rounded-full ${color === c ? 'scale-125' : ''}`} style={{ background: c }} />)}
            </div>
          </div>
          <button type="submit" className="w-full py-1.5 bg-accent hover:bg-accent-hover text-white text-xs rounded-lg">추가</button>
        </form>
      </div>
    </div>
  )
